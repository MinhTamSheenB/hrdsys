...
begin
  ...
  Formdut215:=TFormFhrd215(application.FindComponent('Formfhrd215')); //注意:此處一定要註冊窗体(要不,認不到)
  with Formdut215 do
  begin
    ...
    if PageControl1.ActivePageIndex = 0 then
     ...
     begin
       Excelid.WorkSheets[1].Cells[2,1].Value :='部門代號' ;
       Excelid.WorkSheets[1].Cells[2,2].Value :='部門名稱' ;
       Excelid.WorkSheets[1].Cells[2,3].Value :='人數' ;
       Excelid.WorkSheets[1].Cells[2,4].Value :='加班時數';
     end;
