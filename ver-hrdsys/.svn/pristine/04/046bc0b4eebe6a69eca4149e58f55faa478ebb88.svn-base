object FormDut06: TFormDut06
  Left = 116
  Top = 66
  Width = 796
  Height = 498
  Caption = #21729#24037#29305#27530#21152#29677#20316#26989
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnCreate = FormCreate
  DesignSize = (
    788
    464)
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 788
    Height = 464
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 0
    object PanelTitle: TTntPanel
      Left = 0
      Top = 0
      Width = 788
      Height = 36
      Align = alTop
      BevelOuter = bvNone
      Caption = 'dut06_titl'
      Color = clCream
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -16
      Font.Name = 'Palatino Linotype'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object PanelMain: TPanel
      Left = 0
      Top = 36
      Width = 788
      Height = 428
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 1
      object PanelPre: TPanel
        Left = 0
        Top = 0
        Width = 788
        Height = 81
        Align = alTop
        TabOrder = 0
        object PanelQuery: TTntPanel
          Left = 1
          Top = 1
          Width = 786
          Height = 79
          Align = alClient
          BevelOuter = bvNone
          TabOrder = 0
          object TntGroupBox1: TTntGroupBox
            Left = 0
            Top = 0
            Width = 786
            Height = 79
            Align = alClient
            Caption = 'search_condition'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clRed
            Font.Height = -11
            Font.Name = 'Palatino Linotype'
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            object TntLabel6: TTntLabel
              Left = 5
              Top = 19
              Width = 80
              Height = 13
              Alignment = taRightJustify
              AutoSize = False
              Caption = 'date'
              Font.Charset = ANSI_CHARSET
              Font.Color = clBlue
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ParentFont = False
            end
            object TntLabel7: TTntLabel
              Left = 431
              Top = 19
              Width = 3
              Height = 13
              Caption = '-'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
            end
            object TntLabel9: TTntLabel
              Left = 178
              Top = 19
              Width = 3
              Height = 13
              Caption = '-'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
            end
            object TntLabel2: TTntLabel
              Left = 489
              Top = 48
              Width = 3
              Height = 13
              Caption = '-'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
            end
            object EditEmpBegin: TTntEdit
              Left = 366
              Top = 15
              Width = 60
              Height = 24
              Color = clWhite
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
              MaxLength = 6
              ParentFont = False
              TabOrder = 4
              OnKeyDown = edit_emp_idKeyDown
            end
            object EditEmpEnd: TTntEdit
              Left = 439
              Top = 15
              Width = 60
              Height = 24
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
              MaxLength = 6
              ParentFont = False
              TabOrder = 5
              OnKeyDown = edit_emp_idKeyDown
            end
            object rbEmp: TTntRadioButton
              Left = 280
              Top = 19
              Width = 80
              Height = 17
              Caption = 'by_empid'
              Checked = True
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlue
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ParentFont = False
              TabOrder = 2
              TabStop = True
              OnClick = rbEmpClick
              OnKeyDown = edit_emp_idKeyDown
            end
            object rbDept: TTntRadioButton
              Left = 280
              Top = 51
              Width = 80
              Height = 17
              Caption = 'by_deptcode'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlue
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ParentFont = False
              TabOrder = 3
              OnClick = rbDeptClick
              OnKeyDown = edit_emp_idKeyDown
            end
            object CobDeptBegin: TTntComboBox
              Left = 366
              Top = 44
              Width = 120
              Height = 24
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
              ItemHeight = 16
              ParentFont = False
              TabOrder = 6
              OnKeyDown = edit_emp_idKeyDown
            end
            object CobDeptEnd: TTntComboBox
              Left = 496
              Top = 44
              Width = 120
              Height = 24
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
              ItemHeight = 16
              ParentFont = False
              TabOrder = 7
              OnKeyDown = edit_emp_idKeyDown
            end
            object DTPickBegin: TTntDateTimePicker
              Left = 87
              Top = 15
              Width = 90
              Height = 24
              Date = 38571.000000000000000000
              Time = 38571.000000000000000000
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
              ParentFont = False
              TabOrder = 0
              OnKeyDown = edit_emp_idKeyDown
            end
            object DTPickEnd: TTntDateTimePicker
              Left = 183
              Top = 15
              Width = 90
              Height = 24
              Date = 38573.000000000000000000
              Time = 38573.000000000000000000
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
              ParentFont = False
              TabOrder = 1
              OnKeyDown = edit_emp_idKeyDown
            end
            object TntStaticText1: TTntStaticText
              Left = 500
              Top = 16
              Width = 90
              Height = 20
              Caption = 'dbl_clk_slct_emp'
              Color = clYellow
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlue
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ParentColor = False
              ParentFont = False
              TabOrder = 10
            end
            object Btn_Search: TTntBitBtn
              Left = 696
              Top = 13
              Width = 81
              Height = 25
              Caption = 'find'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ParentFont = False
              TabOrder = 8
              OnClick = Btn_SearchClick
              OnKeyDown = edit_emp_idKeyDown
              Glyph.Data = {
                
                5990555555555555577755555555555555555555555555555555}
              NumGlyphs = 2
            end
            object Btn_Reset: TTntBitBtn
              Left = 696
              Top = 42
              Width = 81
              Height = 25
              Caption = 'reset'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Palatino Linotype'
              Font.Style = []
              ParentFont = False
              TabOrder = 9
              OnClick = Btn_ResetClick
              OnKeyDown = edit_emp_idKeyDown
              Kind = bkRetry
            end
          end
        end
      end
      object PanelGrid: TPanel
        Left = 0
        Top = 81
        Width = 788
        Height = 297
        Align = alClient
        TabOrder = 1
        object DBGrid1: TTntDBGrid
          Left = 1
          Top = 1
          Width = 786
          Height = 295
          Align = alClient
          DataSource = DataSource1
          FixedColor = clSkyBlue
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete]
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnTitleClick = DBGrid1TitleClick
          Columns = <
            item
              Expanded = False
              FieldName = 'emp_id'
              ReadOnly = False
              Width = 58
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'emp_chs'
              Width = 60
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'department'
              ReadOnly = False
              Width = 127
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'clas_name'
              ReadOnly = False
              Width = 81
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'over_date'
              ReadOnly = False
              Width = 71
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'hour_start'
              ReadOnly = False
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'time_start'
              ReadOnly = False
              Width = 60
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'hour_end'
              ReadOnly = False
              Width = 66
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'time_end'
              ReadOnly = False
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'tot_hour'
              ReadOnly = False
              Width = 59
              Visible = True
            end>
        end
      end
      object PanelAct: TTntPanel
        Left = 0
        Top = 378
        Width = 788
        Height = 50
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 2
        object sb_Add: TTntSpeedButton
          Left = 230
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&a)dd'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            88808888888888888880}
          ParentFont = False
          OnClick = sb_AddClick
        end
        object sb_Recover: TTntSpeedButton
          Left = 470
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&u)ndo'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            3333333777333777333333333999993333333333377777333333}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sb_RecoverClick
        end
        object sb_Save: TTntSpeedButton
          Left = 550
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&s)ave'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            0003737FFFFFFFFF7F7330099999999900333777777777777733}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sb_SaveClick
        end
        object sb_edit: TTntSpeedButton
          Left = 390
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&m)odify'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            00333377737FFFFF773333303300000003333337337777777333}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sb_editClick
        end
        object sb_Del: TTntSpeedButton
          Left = 310
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&d)elete'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            F000}
          ParentFont = False
          OnClick = sb_DelClick
        end
        object sb_Print: TTntSpeedButton
          Left = 150
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&p)rint'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            33333337FFFF7733333333300000033333333337777773333333}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sb_PrintClick
        end
      end
    end
  end
  object PanelFlow: TTntPanel
    Left = 251
    Top = 120
    Width = 296
    Height = 305
    Anchors = [akLeft, akTop, akRight, akBottom]
    BevelInner = bvRaised
    BevelOuter = bvNone
    BevelWidth = 2
    BorderWidth = 1
    BorderStyle = bsSingle
    TabOrder = 1
    Visible = False
    object TntLabel3: TTntLabel
      Left = 120
      Top = 12
      Width = 98
      Height = 13
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object PanelFlowTitle: TTntPanel
      Left = 3
      Top = 3
      Width = 286
      Height = 24
      Align = alTop
      Alignment = taLeftJustify
      BevelInner = bvLowered
      BevelOuter = bvNone
      Caption = 'edit_area'
      Color = clGradientActiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindow
      Font.Height = -11
      Font.Name = 'Palatino Linotype'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnMouseDown = PanelFlowTitleMouseDown
      object ImageClose: TImage
        Left = 264
        Top = 1
        Width = 21
        Height = 22
        Align = alRight
        Picture.Data = {
          
          00007F111508713B5A3B1F76045A5A3B560E1F1E04117F000000}
        OnClick = ImageCloseClick
      end
    end
    object TntGroupBox5: TTntGroupBox
      Left = 13
      Top = 37
      Width = 266
      Height = 221
      Align = alClient
      TabOrder = 0
      object Label_emp_chs: TTntLabel
        Left = 2
        Top = 47
        Width = 110
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'emp_chs'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
      end
      object Label_department: TTntLabel
        Left = 2
        Top = 73
        Width = 110
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_department'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
      end
      object Label_over_date: TTntLabel
        Left = 2
        Top = 130
        Width = 110
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'over_date'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
      end
      object Label_hour_start: TTntLabel
        Left = 2
        Top = 157
        Width = 110
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'hour_start'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
      end
      object Label_hour_end: TTntLabel
        Left = 2
        Top = 186
        Width = 110
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'hour_end'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
      end
      object Label_emp_id: TTntLabel
        Left = 2
        Top = 21
        Width = 110
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'emp_id'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
      end
      object Label_class_code: TTntLabel
        Left = 2
        Top = 104
        Width = 110
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'class_code'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
      end
      object sb_FindEmp: TTntSpeedButton
        Left = 182
        Top = 15
        Width = 23
        Height = 22
        Caption = '...'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        OnClick = sb_FindEmpClick
      end
      object edit_name: TTntDBEdit
        Left = 118
        Top = 42
        Width = 80
        Height = 24
        Color = clSkyBlue
        DataField = 'emp_chs'
        DataSource = DataSource1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnKeyDown = edit_emp_idKeyDown
      end
      object edit_dept: TTntDBEdit
        Left = 118
        Top = 68
        Width = 120
        Height = 24
        Color = clSkyBlue
        DataField = 'department'
        DataSource = DataSource1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnKeyDown = edit_emp_idKeyDown
      end
      object DTPickOverdate: TTntDateTimePicker
        Left = 118
        Top = 125
        Width = 90
        Height = 24
        Date = 38573.000000000000000000
        Time = 38573.000000000000000000
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
        ParentFont = False
        TabOrder = 4
        OnChange = DTPickOverdateChange
        OnExit = DTPickOverdateChange
        OnKeyDown = edit_emp_idKeyDown
      end
      object Edit_hour_start: TTntDBEdit
        Left = 118
        Top = 154
        Width = 40
        Height = 24
        DataField = 'hour_start'
        DataSource = DataSource1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        OnChange = Edit_hour_startChange
        OnClick = Edit_hour_startClick
        OnKeyDown = edit_emp_idKeyDown
      end
      object edit_time_start: TTntDBEdit
        Left = 174
        Top = 153
        Width = 40
        Height = 24
        DataField = 'time_start'
        DataSource = DataSource1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        OnChange = Edit_hour_startChange
        OnClick = Edit_hour_startClick
        OnKeyDown = edit_emp_idKeyDown
      end
      object edit_hour_end: TTntDBEdit
        Left = 118
        Top = 182
        Width = 40
        Height = 24
        DataField = 'hour_end'
        DataSource = DataSource1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 7
        OnChange = Edit_hour_startChange
        OnClick = Edit_hour_startClick
        OnKeyDown = edit_emp_idKeyDown
      end
      object edit_time_end: TTntDBEdit
        Left = 174
        Top = 181
        Width = 40
        Height = 24
        DataField = 'time_end'
        DataSource = DataSource1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 8
        OnChange = Edit_hour_startChange
        OnClick = Edit_hour_startClick
        OnKeyDown = edit_emp_idKeyDown
      end
      object edit_emp_id: TTntDBEdit
        Left = 118
        Top = 16
        Width = 60
        Height = 24
        DataField = 'emp_id'
        DataSource = DataSource1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
        ParentFont = False
        TabOrder = 2
        OnKeyDown = edit_emp_idKeyDown
      end
      object CobClass: TTntComboBox
        Left = 118
        Top = 99
        Width = 80
        Height = 24
        Style = csDropDownList
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ItemHeight = 16
        ParentFont = False
        TabOrder = 3
        OnChange = CobClass1Change
        OnExit = CobClass1Change
        OnKeyDown = edit_emp_idKeyDown
      end
    end
    object Panel2: TPanel
      Left = 3
      Top = 27
      Width = 286
      Height = 10
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 2
    end
    object Panel4: TPanel
      Left = 3
      Top = 37
      Width = 10
      Height = 221
      Align = alLeft
      BevelOuter = bvNone
      TabOrder = 3
    end
    object Panel5: TPanel
      Left = 279
      Top = 37
      Width = 10
      Height = 221
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 4
    end
    object Panel3: TPanel
      Left = 3
      Top = 258
      Width = 286
      Height = 40
      Align = alBottom
      BevelOuter = bvNone
      TabOrder = 5
      object DBNavigator1: TDBNavigator
        Left = 39
        Top = 8
        Width = 112
        Height = 25
        DataSource = DataSource1
        VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
        Flat = True
        TabOrder = 0
        Visible = False
      end
      object bbtn_add: TTntBitBtn
        Left = 88
        Top = 8
        Width = 65
        Height = 25
        Caption = 'add'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnClick = bbtn_addClick
        Glyph.Data = {
          
          88808888888888888880}
      end
      object bbtn_ok: TTntBitBtn
        Left = 162
        Top = 8
        Width = 65
        Height = 25
        Caption = 'finish'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        OnClick = bbtn_okClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
    end
  end
  object ADOQuery1: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    AfterOpen = ADOQuery1AfterOpen
    BeforeClose = ADOQuery1BeforeClose
    AfterClose = ADOQuery1AfterClose
    BeforePost = ADOQuery1BeforePost
    BeforeDelete = ADOQuery1BeforeDelete
    AfterScroll = ADOQuery1AfterScroll
    OnWillChangeRecord = ADOQuery1WillChangeRecord
    Parameters = <>
    SQL.Strings = (
      'select * from hrd_dut_spe_overwork where emp_id=-1')
    Left = 200
    object ADOQuery1updator: TWideStringField
      FieldName = 'updator'
      Size = 10
    end
    object ADOQuery1update_time: TDateTimeField
      FieldName = 'update_time'
    end
    object ADOQuery1emp_id: TStringField
      DisplayWidth = 6
      FieldName = 'emp_id'
      OnChange = ADOQuery1emp_idChange
      FixedChar = True
      Size = 6
    end
    object ADOQuery1dept_code: TStringField
      FieldName = 'dept_code'
      OnChange = ADOQuery1emp_idChange
      Size = 50
    end
    object ADOQuery1emp_chs: TTntWideStringField
      FieldKind = fkLookup
      FieldName = 'emp_chs'
      LookupDataSet = ADOQuery2
      LookupKeyFields = 'emp_id'
      LookupResultField = 'emp_chs'
      KeyFields = 'emp_id'
      Lookup = True
    end
    object ADOQuery1department: TTntWideStringField
      FieldKind = fkLookup
      FieldName = 'department'
      LookupDataSet = ADOQuery2
      LookupKeyFields = 'emp_id'
      LookupResultField = 'dept_name'
      KeyFields = 'emp_id'
      Lookup = True
    end
    object ADOQuery1class_code: TStringField
      FieldName = 'class_code'
      OnChange = ADOQuery1emp_idChange
      FixedChar = True
      Size = 2
    end
    object ADOQuery1clas_name: TTntWideStringField
      FieldKind = fkLookup
      FieldName = 'clas_name'
      LookupDataSet = ADOQuery3
      LookupKeyFields = 'clas_code'
      LookupResultField = 'clas_name'
      KeyFields = 'class_code'
      Lookup = True
    end
    object ADOQuery1over_date: TDateTimeField
      FieldName = 'over_date'
      OnChange = ADOQuery1emp_idChange
    end
    object ADOQuery1hour_start: TStringField
      FieldName = 'hour_start'
      OnChange = ADOQuery1emp_idChange
      FixedChar = True
      Size = 2
    end
    object ADOQuery1time_start: TStringField
      FieldName = 'time_start'
      OnChange = ADOQuery1emp_idChange
      FixedChar = True
      Size = 2
    end
    object ADOQuery1hour_end: TStringField
      FieldName = 'hour_end'
      OnChange = ADOQuery1emp_idChange
      FixedChar = True
      Size = 2
    end
    object ADOQuery1time_end: TStringField
      FieldName = 'time_end'
      OnChange = ADOQuery1emp_idChange
      FixedChar = True
      Size = 2
    end
    object ADOQuery1tot_hour: TBCDField
      FieldName = 'tot_hour'
      Precision = 18
      Size = 5
    end
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    Left = 232
  end
  object ADOQuery2: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select A.emp_id,A.emp_chs,A.dept_code,B.dept_code+'#39'-'#39'+B.abbr_tit' +
        'l as dept_name from hrd_emp as A,hrd_dept as B where A.dept_code' +
        '=B.dept_code ')
    Left = 269
    Top = 5
  end
  object ADOQuery3: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select clas_code,clas_code+'#39'-'#39'+clas_chs as clas_name from hrd_du' +
        't_class')
    Left = 297
    Top = 5
  end
end
