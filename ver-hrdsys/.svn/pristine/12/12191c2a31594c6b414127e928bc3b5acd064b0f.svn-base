...
type
  ...
  private
    //2014.10.23 檢查事假是否超過25天
    function IsLeave25(AEmpID: string): Boolean;
...
begin
  ...
  if (ask_month<=LockMonthPart) then
  begin
...
//  if GetYearHol(emp_id,FormatDateTime('yyyy',DTPickSdate.Date)).Strings[3]
  ...
  //
  if not IsLeave25(emp_id) then
  begin
    if WideMessageDlg('員工事假天數已超過25天，是否繼續？',mtConfirmation,[mbYes, mbNo],0)=mrno then
      Abort;
  end;
  //
  with ADOQuery1 do
...
//2014.10.23 檢查事假是否超過25天
function TFormDut02.IsLeave25(AEmpID: string): Boolean;
var
  sSQL: string;
  //
  I: Integer;
  //
  sHours, sFieldName: string;
  //
  dHours: Double;
begin
  dHours := 0;
  ...
  sSQL := 'SELECT *'
        ...
        + '   AND DUT_MON = '+QuotedStr(FormatDateTime('YYYYMM', ADOQuery1.FieldByName('s_ask_d').AsDateTime))
        + '   AND CLAS_CODE = ''12''';
  //
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //
        if (Trim(FieldByName(sFieldName).AsString)= '') or FieldByName(sFieldName).IsNull then
          Continue;
        //
        sHours:= FieldByName('day'+Format('%.2d', [I])).AsString;
        //
        if sHours= '' then
          dHours:= 0
        else
          dHours:= dHours + StrToIntDef(Copy(sHours, 1, 2), 0)+ StrToIntDef(Copy(sHours, 3, 2), 0)/60;
      end;
      //
      Next;
    end;
  ...
  //
  Result := (dHours/8 + ADOQuery1tot_hour.AsFloat)<= 25;
end;
