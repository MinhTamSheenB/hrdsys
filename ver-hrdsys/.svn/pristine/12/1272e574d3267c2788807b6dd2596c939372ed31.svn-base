object formDut05: TformDut05
  Left = 74
  Top = 141
  BorderStyle = bsSingle
  Caption = 'formDut05'
  ClientHeight = 464
  ClientWidth = 790
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnClose = FormClose
  OnCreate = FormCreate
  DesignSize = (
    790
    464)
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TTntPanel
    Left = 0
    Top = 0
    Width = 790
    Height = 464
    Align = alClient
    TabOrder = 0
    object PanelTitle: TTntPanel
      Left = 1
      Top = 1
      Width = 788
      Height = 33
      Align = alTop
      Caption = 'dut05_titl'
      Color = clCream
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -16
      Font.Name = 'Palatino Linotype'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object PanelMain: TTntPanel
      Left = 1
      Top = 34
      Width = 788
      Height = 429
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 1
      object TntGroupBox2: TTntGroupBox
        Left = 0
        Top = 0
        Width = 788
        Height = 80
        Align = alTop
        Caption = 'search_condition'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object DEPT_B: TTntComboBox
          Left = 334
          Top = 43
          Width = 151
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          ItemHeight = 13
          ParentFont = False
          TabOrder = 7
          OnKeyDown = TEDT_IDKeyDown
        end
        object tbb_search: TTntBitBtn
          Left = 672
          Top = 13
          Width = 73
          Height = 25
          Caption = 'ok'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
          TabOrder = 9
          OnClick = tbb_searchClick
          OnKeyDown = TEDT_IDKeyDown
          Glyph.Data = {
            
            5990555555555555577755555555555555555555555555555555}
          NumGlyphs = 2
        end
        object tbb_reset: TTntBitBtn
          Left = 672
          Top = 42
          Width = 73
          Height = 25
          Caption = 'reset'
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
          TabOrder = 10
          OnClick = tbb_resetClick
          OnKeyDown = TEDT_IDKeyDown
          Kind = bkRetry
        end
        object date_s1: TTntDateTimePicker
          Left = 17
          Top = 43
          Width = 89
          Height = 21
          Date = 38575.000000000000000000
          Time = 38575.000000000000000000
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          TabOrder = 1
          OnKeyDown = TEDT_IDKeyDown
        end
        object date_e1: TTntDateTimePicker
          Left = 112
          Top = 43
          Width = 89
          Height = 21
          Date = 38575.000000000000000000
          Time = 38575.000000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          ParentFont = False
          TabOrder = 2
          OnKeyDown = TEDT_IDKeyDown
        end
        object DEPT_E: TTntComboBox
          Left = 494
          Top = 43
          Width = 152
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          ItemHeight = 13
          ParentFont = False
          TabOrder = 8
          OnKeyDown = TEDT_IDKeyDown
        end
        object id_s1: TTntEdit
          Left = 334
          Top = 14
          Width = 57
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          MaxLength = 6
          ParentFont = False
          TabOrder = 4
          Text = '000000'
          OnKeyDown = TEDT_IDKeyDown
        end
        object id_e1: TTntEdit
          Left = 398
          Top = 14
          Width = 57
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          MaxLength = 6
          ParentFont = False
          TabOrder = 5
          Text = '999999'
          OnKeyDown = TEDT_IDKeyDown
        end
        object tcb_date: TTntCheckBox
          Left = 18
          Top = 20
          Width = 97
          Height = 17
          Caption = 'trip_date'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 0
          OnClick = tcb_dateClick
          OnKeyDown = TEDT_IDKeyDown
        end
        object tcb_id: TTntCheckBox
          Left = 240
          Top = 20
          Width = 89
          Height = 17
          Caption = 'fld_emp_id'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 3
          OnClick = tcb_dateClick
          OnKeyDown = TEDT_IDKeyDown
        end
        object tcb_dept: TTntCheckBox
          Left = 240
          Top = 46
          Width = 89
          Height = 17
          Caption = 'fld_dept_code'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 6
          OnClick = tcb_dateClick
          OnKeyDown = TEDT_IDKeyDown
        end
        object TntStaticText2: TTntStaticText
          Left = 460
          Top = 15
          Width = 90
          Height = 20
          Caption = 'dbl_clk_slct_emp'
          Color = clYellow
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentColor = False
          ParentFont = False
          TabOrder = 11
        end
      end
      object TntPanel6: TTntPanel
        Left = 0
        Top = 374
        Width = 788
        Height = 55
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 1
        object sb_Add: TTntSpeedButton
          Left = 230
          Top = 13
          Width = 80
          Height = 25
          Caption = '(&a)dd'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            88808888888888888880}
          ParentFont = False
          OnClick = sb_AddClick
        end
        object sb_Recover: TTntSpeedButton
          Left = 469
          Top = 13
          Width = 80
          Height = 25
          Caption = '(&u)ndo'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            3333333777333777333333333999993333333333377777333333}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sb_RecoverClick
        end
        object sb_Save: TTntSpeedButton
          Left = 549
          Top = 13
          Width = 80
          Height = 25
          Caption = '(&s)ave'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            0003737FFFFFFFFF7F7330099999999900333777777777777733}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sb_SaveClick
        end
        object sb_edit: TTntSpeedButton
          Left = 390
          Top = 13
          Width = 80
          Height = 25
          Caption = '(&m)odify'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            00333377737FFFFF773333303300000003333337337777777333}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sb_editClick
        end
        object sb_Del: TTntSpeedButton
          Left = 310
          Top = 13
          Width = 80
          Height = 25
          Caption = '(&d)elete'
          Enabled = False
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          Glyph.Data = {
            
            F000}
          ParentFont = False
          OnClick = sb_DelClick
        end
        object sb_Print: TTntSpeedButton
          Left = 150
          Top = 13
          Width = 80
          Height = 25
          Caption = '(&p)rint'
          Enabled = False
          Flat = True
          Glyph.Data = {
            
            33333337FFFF7733333333300000033333333337777773333333}
          NumGlyphs = 2
          OnClick = sb_PrintClick
        end
      end
      object TntGroupBox3: TTntGroupBox
        Left = 0
        Top = 80
        Width = 788
        Height = 294
        Align = alClient
        Caption = 'trip_detail'
        Color = clBtnFace
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        TabOrder = 2
        object TntDBGrid2: TTntDBGrid
          Left = 2
          Top = 18
          Width = 784
          Height = 274
          Align = alClient
          DataSource = DataSource2
          FixedColor = clSkyBlue
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete]
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnTitleClick = TntDBGrid1TitleClick
          Columns = <
            item
              Expanded = False
              FieldName = 'trip_seg'
              Width = 54
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'emp_id'
              Width = 60
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'emp_chs'
              Width = 67
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'dept_code'
              Width = 147
              Visible = True
            end
            item
              Expanded = False
              FieldName = 's_trip_d'
              Width = 66
              Visible = True
            end
            item
              Expanded = False
              FieldName = 's_datetime'
              Width = 60
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'e_trip_d'
              Width = 47
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'e_datetime'
              Width = 61
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'tot_hour'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'reason'
              Width = 180
              Visible = True
            end>
        end
      end
    end
  end
  object PanelFlow: TTntPanel
    Left = 104
    Top = 144
    Width = 401
    Height = 253
    Anchors = [akLeft, akTop, akRight, akBottom]
    BevelInner = bvRaised
    BevelOuter = bvNone
    BevelWidth = 2
    BorderWidth = 1
    BorderStyle = bsSingle
    TabOrder = 1
    Visible = False
    object TntLabel10: TTntLabel
      Left = 120
      Top = 12
      Width = 98
      Height = 13
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object PanelFlowTitle: TTntPanel
      Left = 3
      Top = 3
      Width = 391
      Height = 24
      Align = alTop
      Alignment = taLeftJustify
      BevelInner = bvLowered
      BevelOuter = bvNone
      Caption = 'edit_area'
      Color = clGradientActiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindow
      Font.Height = -11
      Font.Name = 'Palatino Linotype'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnMouseDown = PanelFlowTitleMouseDown
      object ImageClose: TImage
        Left = 369
        Top = 1
        Width = 21
        Height = 22
        Align = alRight
        Picture.Data = {
          
          00007F111508713B5A3B1F76045A5A3B560E1F1E04117F000000}
        OnClick = ImageCloseClick
      end
    end
    object TntGroupBox1: TTntGroupBox
      Left = 13
      Top = 37
      Width = 371
      Height = 169
      Align = alClient
      TabOrder = 0
      object TntLabel11: TTntLabel
        Left = 5
        Top = 18
        Width = 90
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'emp_id'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object sb_FindEmp: TTntSpeedButton
        Left = 167
        Top = 14
        Width = 23
        Height = 22
        Caption = '...'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        OnClick = sb_FindEmpClick
      end
      object Label_s_ask_d: TTntLabel
        Left = 5
        Top = 96
        Width = 90
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_ask_d'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label_s_ask_h: TTntLabel
        Left = 5
        Top = 122
        Width = 90
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_ask_t'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel13: TTntLabel
        Left = 5
        Top = 45
        Width = 90
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'emp_chs'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -13
        Font.Name = 'MS PGothic'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel14: TTntLabel
        Left = 5
        Top = 69
        Width = 90
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'department'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -13
        Font.Name = 'MS PGothic'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel15: TTntLabel
        Left = 136
        Top = 124
        Width = 12
        Height = 13
        Caption = #65306
      end
      object TntLabel16: TTntLabel
        Left = 236
        Top = 126
        Width = 12
        Height = 13
        Caption = #65306
      end
      object TntLabel17: TTntLabel
        Left = 186
        Top = 122
        Width = 6
        Height = 13
        Alignment = taCenter
        Caption = '--'
      end
      object TntLabel18: TTntLabel
        Left = 186
        Top = 95
        Width = 6
        Height = 13
        Alignment = taCenter
        Caption = '--'
      end
      object TntLabel1: TTntLabel
        Left = 5
        Top = 146
        Width = 90
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_reason'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object dtp_DateBeg: TTntDateTimePicker
        Left = 100
        Top = 92
        Width = 87
        Height = 21
        Date = 38573.000000000000000000
        Time = 38573.000000000000000000
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        ParseInput = True
        TabOrder = 1
        OnKeyDown = TEDT_IDKeyDown
      end
      object dtp_DateEnd: TTntDateTimePicker
        Left = 200
        Top = 92
        Width = 87
        Height = 21
        Date = 38573.000000000000000000
        Time = 38573.000000000000000000
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        TabOrder = 2
        OnKeyDown = TEDT_IDKeyDown
      end
      object TEDT_ID: TTntDBEdit
        Left = 100
        Top = 16
        Width = 60
        Height = 21
        DataField = 'emp_id'
        DataSource = DataSource2
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        TabOrder = 0
        OnChange = TEDT_IDChange
        OnKeyDown = TEDT_IDKeyDown
      end
      object s_hour: TTntDBEdit
        Left = 100
        Top = 118
        Width = 36
        Height = 21
        DataField = 's_trip_h'
        DataSource = DataSource2
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        TabOrder = 3
        OnChange = s_hourChange
        OnKeyDown = TEDT_IDKeyDown
      end
      object s_minute: TTntDBEdit
        Left = 148
        Top = 118
        Width = 36
        Height = 21
        DataField = 's_trip_m'
        DataSource = DataSource2
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        TabOrder = 4
        OnChange = s_hourChange
        OnKeyDown = TEDT_IDKeyDown
      end
      object e_hour: TTntDBEdit
        Left = 200
        Top = 118
        Width = 36
        Height = 21
        DataField = 'e_trip_h'
        DataSource = DataSource2
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        TabOrder = 5
        OnChange = s_hourChange
        OnKeyDown = TEDT_IDKeyDown
      end
      object e_minute: TTntDBEdit
        Left = 248
        Top = 118
        Width = 36
        Height = 21
        DataField = 'e_trip_m'
        DataSource = DataSource2
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        TabOrder = 6
        OnChange = s_hourChange
        OnKeyDown = TEDT_IDKeyDown
      end
      object name: TTntEdit
        Left = 100
        Top = 40
        Width = 81
        Height = 21
        Color = clSkyBlue
        ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
        ReadOnly = True
        TabOrder = 7
        OnKeyDown = TEDT_IDKeyDown
      end
      object dept: TTntEdit
        Left = 100
        Top = 65
        Width = 137
        Height = 21
        Color = clSkyBlue
        ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
        ReadOnly = True
        TabOrder = 8
        OnKeyDown = TEDT_IDKeyDown
      end
      object TntDBEdit1: TTntDBEdit
        Left = 100
        Top = 144
        Width = 261
        Height = 21
        DataField = 'reason'
        DataSource = DataSource2
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        TabOrder = 9
        OnChange = TEDT_IDChange
        OnKeyDown = TEDT_IDKeyDown
      end
    end
    object Panel6: TPanel
      Left = 3
      Top = 27
      Width = 391
      Height = 10
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 2
    end
    object Panel7: TPanel
      Left = 3
      Top = 206
      Width = 391
      Height = 40
      Align = alBottom
      BevelOuter = bvNone
      TabOrder = 3
      object DBNavigator2: TDBNavigator
        Left = 31
        Top = 8
        Width = 112
        Height = 25
        DataSource = DataSource1
        VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
        Flat = True
        TabOrder = 0
        Visible = False
      end
      object bbtn_add: TTntBitBtn
        Left = 80
        Top = 8
        Width = 65
        Height = 25
        Caption = 'add'
        TabOrder = 1
        OnClick = bbtn_addClick
        Glyph.Data = {
          
          88808888888888888880}
      end
      object bbtn_OK: TTntBitBtn
        Left = 154
        Top = 8
        Width = 65
        Height = 25
        Caption = 'finish'
        TabOrder = 2
        OnClick = bbtn_OKClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
    end
    object Panel8: TPanel
      Left = 3
      Top = 37
      Width = 10
      Height = 169
      Align = alLeft
      BevelOuter = bvNone
      TabOrder = 4
    end
    object Panel9: TPanel
      Left = 384
      Top = 37
      Width = 10
      Height = 169
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 5
    end
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    Left = 64
    Top = 8
  end
  object ADOQuery1: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <>
    Left = 32
    Top = 8
  end
  object ADOQuery3: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <>
    SQL.Strings = (
      'select a.emp_id,a.emp_chs,a.dept_code+'#39'-'#39'+b.abbr_titl as dept'
      ' from hrd_emp as a,hrd_dept  as b '
      'where a.dept_code=b.dept_code')
    Left = 544
    Top = 24
  end
  object DataSource3: TDataSource
    DataSet = ADOQuery3
    Left = 573
    Top = 24
  end
  object ADOQuery2: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    AfterOpen = ADOQuery2AfterOpen
    BeforeClose = ADOQuery2BeforeClose
    BeforePost = ADOQuery2BeforePost
    AfterPost = ADOQuery2AfterPost
    BeforeDelete = ADOQuery2BeforeDelete
    AfterDelete = ADOQuery2AfterDelete
    AfterScroll = ADOQuery2AfterScroll
    OnCalcFields = ADOQuery2CalcFields
    Parameters = <>
    SQL.Strings = (
      'SELECT         *'
      'FROM             hrd_dut_trip ')
    Left = 605
    Top = 26
    object ADOQuery2trip_seg: TLargeintField
      FieldName = 'trip_seg'
      ReadOnly = True
    end
    object ADOQuery2emp_id: TStringField
      FieldName = 'emp_id'
      Size = 6
    end
    object ADOQuery2s_trip_d: TDateTimeField
      FieldName = 's_trip_d'
    end
    object ADOQuery2s_trip_h: TStringField
      FieldName = 's_trip_h'
      Size = 2
    end
    object ADOQuery2s_trip_m: TStringField
      FieldName = 's_trip_m'
      Size = 2
    end
    object ADOQuery2e_trip_d: TDateTimeField
      FieldName = 'e_trip_d'
    end
    object ADOQuery2e_trip_h: TStringField
      FieldName = 'e_trip_h'
      Size = 2
    end
    object ADOQuery2e_trip_m: TStringField
      FieldName = 'e_trip_m'
      Size = 2
    end
    object ADOQuery2reason: TWideStringField
      FieldName = 'reason'
      Size = 100
    end
    object ADOQuery2emp_chs: TStringField
      FieldKind = fkLookup
      FieldName = 'emp_chs'
      LookupDataSet = ADOQuery4
      LookupKeyFields = 'emp_id'
      LookupResultField = 'emp_chs'
      KeyFields = 'emp_id'
      Size = 10
      Lookup = True
    end
    object ADOQuery2dept_code: TStringField
      FieldKind = fkLookup
      FieldName = 'dept_code'
      LookupDataSet = ADOQuery4
      LookupKeyFields = 'emp_id'
      LookupResultField = 'dept_code'
      KeyFields = 'emp_id'
      Size = 30
      Lookup = True
    end
    object ADOQuery2upd_user: TStringField
      FieldName = 'upd_user'
      Size = 16
    end
    object ADOQuery2upd_date: TDateTimeField
      FieldName = 'upd_date'
    end
    object ADOQuery2s_datetime: TStringField
      DisplayWidth = 20
      FieldKind = fkCalculated
      FieldName = 's_datetime'
      Calculated = True
    end
    object ADOQuery2e_datetime: TStringField
      FieldKind = fkCalculated
      FieldName = 'e_datetime'
      Calculated = True
    end
    object ADOQuery2tot_hour: TStringField
      FieldKind = fkCalculated
      FieldName = 'tot_hour'
      Size = 4
      Calculated = True
    end
  end
  object DataSource2: TDataSource
    DataSet = ADOQuery2
    Left = 645
    Top = 26
  end
  object ADOQuery4: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select a.emp_id, a.emp_chs,b.dept_code+'#39'-'#39'+b.abbr_titl as dept_c' +
        'ode from'
      'hrd_emp a ,hrd_dept b where a.dept_code=b.dept_code ')
    Left = 676
    Top = 21
  end
end
