...
uses
  ...
  TntButtons, Menus, TntMenus, QuickRpt, ComObj, ExcelXP;
...
type
  ...
  private
    procedure ProduceXLSFile(xlsFileName: string);
...
procedure TFormPrtSet.ProduceXLSFile(xlsFileName: string);
var
...
begin
  try
    ...
    Application.MessageBox('Microsoft excel had not been installed','Information',MB_OK);
    Exit;
  end;

  try
    WorkBook := excelApp.WorkBooks.Add;
    //excelApp.ActiveSheet.WrapText := False;
    for j := 1 to MemoTitle.Lines.Count  do
    begin
      excelApp.Cells(j,1) := MemoTitle.Text;
    end;
    ...
    begin
      if ChkFieldList.Checked[i] then
      begin
        excelApp.Cells(j,k) := ChkFieldList.Items[i];
        inc(k);
      end;
    end;
    inc(j);
    ...
    //    excelApp.Cells(j,i+1) := DBGrid1.Columns[i].Title.Caption;
    //end;
    ...
    begin
      ...
      begin
        if DBGrid1.Columns[i].Visible then
        begin
          excelApp.Cells(j,k) := DBGrid1.Fields[i].Value;
          Inc(k);
        end;
      end;
    ...
    ShowMessage('Excel file is generated failure.');
    Exit;
  end;
  Application.MessageBox('Excel file is generated success.','Information',MB_OK+MB_ICONINFORMATION);
