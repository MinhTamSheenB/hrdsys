object Formfhrd207: TFormfhrd207
  Left = 250
  Top = 155
  Width = 774
  Height = 480
  Caption = 'Formfhrd207'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object TntPanel1: TTntPanel
    Left = 0
    Top = 0
    Width = 766
    Height = 36
    Align = alTop
    Caption = 'dut08_titl'
    Color = clCream
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -16
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
  end
  object TntPanel2: TTntPanel
    Left = 0
    Top = 36
    Width = 766
    Height = 410
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 1
    object TntPanel5: TTntPanel
      Left = 0
      Top = 0
      Width = 766
      Height = 169
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 0
      object TntPanel4: TTntPanel
        Left = 614
        Top = 0
        Width = 152
        Height = 169
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 0
        object GroupBox1: TGroupBox
          Left = 0
          Top = 0
          Width = 152
          Height = 169
          Align = alClient
          Caption = 'Log'
          TabOrder = 0
          object MemoLog: TMemo
            Left = 2
            Top = 15
            Width = 148
            Height = 127
            Align = alClient
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            ReadOnly = True
            TabOrder = 0
          end
          object Panel1: TPanel
            Left = 2
            Top = 142
            Width = 148
            Height = 25
            Align = alBottom
            BevelOuter = bvNone
            TabOrder = 1
            object BtnClear: TBitBtn
              Left = 41
              Top = 0
              Width = 75
              Height = 25
              Caption = 'Clear Log'
              TabOrder = 0
              Kind = bkRetry
            end
          end
        end
      end
      object TntPanel3: TTntPanel
        Left = 0
        Top = 0
        Width = 614
        Height = 169
        Align = alClient
        TabOrder = 1
        object TntPanel7: TTntPanel
          Left = 1
          Top = 1
          Width = 612
          Height = 130
          Align = alClient
          BevelOuter = bvNone
          TabOrder = 0
          object TntLabel2: TTntLabel
            Left = 324
            Top = 103
            Width = 25
            Height = 13
            AutoSize = False
            Caption = '-------'
          end
          object TntLabel3: TTntLabel
            Left = 56
            Top = 11
            Width = 89
            Height = 13
            Alignment = taRightJustify
            AutoSize = False
            Caption = 'fld_wrk_date'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clRed
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
          end
          object rb_id: TTntRadioButton
            Left = 30
            Top = 41
            Width = 116
            Height = 17
            Caption = 'fld_emp_id'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            OnClick = rb_idClick
          end
          object rb_dept: TTntRadioButton
            Left = 30
            Top = 76
            Width = 116
            Height = 17
            Caption = 'dept_range'
            Checked = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            TabStop = True
            OnClick = rb_deptClick
          end
          object cb_trt: TTntCheckBox
            Left = 30
            Top = 103
            Width = 166
            Height = 17
            Caption = 'dut08_conk'
            Enabled = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 2
            OnClick = cb_trtClick
          end
          object dept_s: TTntComboBox
            Left = 150
            Top = 73
            Width = 185
            Height = 21
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            ItemHeight = 13
            TabOrder = 3
          end
          object dept_e: TTntComboBox
            Left = 339
            Top = 73
            Width = 185
            Height = 21
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            ItemHeight = 13
            TabOrder = 4
          end
          object s_date: TTntDateTimePicker
            Left = 204
            Top = 101
            Width = 81
            Height = 21
            Date = 38581.421148530090000000
            Time = 38581.421148530090000000
            Enabled = False
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            TabOrder = 5
          end
          object e_date: TTntDateTimePicker
            Left = 346
            Top = 101
            Width = 81
            Height = 21
            Date = 38581.421148530090000000
            Time = 38581.421148530090000000
            Enabled = False
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            TabOrder = 6
          end
          object e_time: TMaskEdit
            Left = 429
            Top = 101
            Width = 39
            Height = 21
            AutoSize = False
            Enabled = False
            EditMask = '!90:00;1;_'
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            MaxLength = 5
            TabOrder = 7
            Text = '  :  '
          end
          object id_s: TTntEdit
            Left = 150
            Top = 39
            Width = 45
            Height = 21
            Enabled = False
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            MaxLength = 6
            TabOrder = 8
            Text = '000000'
            OnChange = id_sChange
          end
          object id_e: TTntEdit
            Left = 202
            Top = 39
            Width = 45
            Height = 21
            Enabled = False
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            MaxLength = 6
            TabOrder = 9
            Text = '999999'
          end
          object s_time: TMaskEdit
            Left = 286
            Top = 101
            Width = 39
            Height = 21
            AutoSize = False
            Enabled = False
            EditMask = '!90:00;1;_'
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            MaxLength = 5
            TabOrder = 10
            Text = '  :  '
          end
          object att_date: TTntDateTimePicker
            Left = 149
            Top = 6
            Width = 82
            Height = 21
            Date = 38581.000000000000000000
            Time = 38581.000000000000000000
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            TabOrder = 11
            OnChange = att_dateChange
          end
          object att_date_end: TTntDateTimePicker
            Left = 237
            Top = 6
            Width = 82
            Height = 21
            Date = 38581.999988425930000000
            Time = 38581.999988425930000000
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            TabOrder = 12
          end
          object Panel2: TPanel
            Left = 512
            Top = 0
            Width = 100
            Height = 130
            Align = alRight
            BevelOuter = bvNone
            TabOrder = 13
            object GroupBox2: TGroupBox
              Left = 0
              Top = 0
              Width = 100
              Height = 130
              Align = alClient
              Caption = #37096#20998#36681#27284
              TabOrder = 0
              Visible = False
              object chk_ask: TTntCheckBox
                Left = 8
                Top = 24
                Width = 65
                Height = 17
                Caption = #35531#20551
                TabOrder = 0
              end
              object chk_over: TTntCheckBox
                Left = 8
                Top = 48
                Width = 65
                Height = 17
                Caption = #21152#29677
                TabOrder = 1
              end
              object chk_stop: TTntCheckBox
                Left = 8
                Top = 72
                Width = 65
                Height = 17
                Caption = #20572#24037
                TabOrder = 2
              end
              object chk_trip: TTntCheckBox
                Left = 8
                Top = 96
                Width = 65
                Height = 17
                Caption = #20844#20986
                TabOrder = 3
              end
            end
          end
          object CobDutSts: TTntComboBox
            Left = 376
            Top = 40
            Width = 145
            Height = 21
            Style = csDropDownList
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            ItemHeight = 13
            TabOrder = 14
          end
        end
        object TntPanel10: TTntPanel
          Left = 1
          Top = 131
          Width = 612
          Height = 37
          Align = alBottom
          BevelOuter = bvNone
          TabOrder = 1
          object bb_tran: TTntBitBtn
            Left = 112
            Top = 5
            Width = 97
            Height = 25
            Caption = 'transferfiles'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            OnClick = bb_tranClick
            Kind = bkRetry
          end
          object Tbb_reset: TTntBitBtn
            Left = 230
            Top = 5
            Width = 97
            Height = 25
            Caption = 'reset'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            OnClick = Tbb_resetClick
            Kind = bkNo
          end
          object BtnQuery: TTntBitBtn
            Left = 351
            Top = 5
            Width = 97
            Height = 25
            Caption = 'query'
            Default = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ModalResult = 1
            ParentFont = False
            TabOrder = 2
            OnClick = BtnQueryClick
            Glyph.Data = {
              
              8E689F675BA5686BFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
          end
        end
      end
    end
    object TntPanel8: TTntPanel
      Left = 0
      Top = 169
      Width = 766
      Height = 195
      Align = alClient
      TabOrder = 1
      object TntDBGrid1: TTntDBGrid
        Left = 1
        Top = 1
        Width = 764
        Height = 193
        Align = alClient
        DataSource = DataSource1
        FixedColor = clSkyBlue
        ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'wrk_date'
            Width = 80
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'emp_id'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'seg_code'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'clas_code'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'clas_name'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 's_start'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 's_end'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'real_start'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'real_end'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'wrk_sts'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'late'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'early'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'local_sts'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'over_flag'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'notify'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'upd_user'
            Visible = False
          end
          item
            Expanded = False
            FieldName = 'upd_date'
            Visible = False
          end>
      end
    end
    object TntPanel11: TTntPanel
      Left = 0
      Top = 364
      Width = 766
      Height = 46
      Align = alBottom
      TabOrder = 2
      object TntLabel4: TTntLabel
        Left = 68
        Top = 6
        Width = 121
        Height = 13
        AutoSize = False
        Caption = #27491#22312#34389#29702#30340#24037#34399
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel5: TTntLabel
        Left = 68
        Top = 26
        Width = 121
        Height = 13
        AutoSize = False
        Caption = #25552#31034#20449#24687
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object ProgressBar1: TProgressBar
        Left = 400
        Top = 3
        Width = 369
        Height = 16
        TabOrder = 0
      end
      object sb_emp: TTntStatusBar
        Left = 208
        Top = 2
        Width = 56
        Height = 19
        Align = alNone
        Color = clSkyBlue
        Panels = <>
      end
      object sb_hint: TTntStatusBar
        Left = 208
        Top = 23
        Width = 560
        Height = 19
        Align = alNone
        Color = clSkyBlue
        Panels = <>
      end
      object sb_recno: TTntStatusBar
        Left = 296
        Top = 2
        Width = 73
        Height = 19
        Align = alNone
        Panels = <>
      end
    end
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    Left = 256
    Top = 8
  end
  object ADOQuery1: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    CommandTimeout = 0
    Parameters = <>
    SQL.Strings = (
      'select top 1 * from hrd_dut_day A')
    Left = 208
    Top = 8
    object ADOQuery1upd_user: TStringField
      FieldName = 'upd_user'
      Size = 16
    end
    object ADOQuery1upd_date: TDateTimeField
      FieldName = 'upd_date'
    end
    object ADOQuery1wrk_date: TDateTimeField
      FieldName = 'wrk_date'
    end
    object ADOQuery1emp_id: TStringField
      FieldName = 'emp_id'
      FixedChar = True
      Size = 6
    end
    object ADOQuery1seg_code: TStringField
      FieldName = 'seg_code'
      FixedChar = True
      Size = 2
    end
    object ADOQuery1clas_code: TStringField
      FieldName = 'clas_code'
      FixedChar = True
      Size = 2
    end
    object ADOQuery1s_start: TStringField
      FieldName = 's_start'
      FixedChar = True
      Size = 4
    end
    object ADOQuery1s_end: TStringField
      FieldName = 's_end'
      FixedChar = True
      Size = 4
    end
    object ADOQuery1real_start: TStringField
      FieldName = 'real_start'
      FixedChar = True
      Size = 4
    end
    object ADOQuery1real_end: TStringField
      FieldName = 'real_end'
      FixedChar = True
      Size = 4
    end
    object ADOQuery1wrk_sts: TStringField
      FieldName = 'wrk_sts'
      FixedChar = True
      Size = 2
    end
    object ADOQuery1late: TStringField
      FieldName = 'late'
      FixedChar = True
      Size = 2
    end
    object ADOQuery1early: TStringField
      FieldName = 'early'
      FixedChar = True
      Size = 2
    end
    object ADOQuery1over_flag: TStringField
      FieldName = 'over_flag'
      FixedChar = True
      Size = 1
    end
    object ADOQuery1notify: TStringField
      FieldName = 'notify'
      FixedChar = True
      Size = 1
    end
    object ADOQuery1local_sts: TStringField
      FieldName = 'local_sts'
      Size = 2
    end
  end
  object ADOCommand1: TADOCommand
    CommandTimeout = 0
    Connection = DMHrdsys.conn
    Parameters = <>
    Left = 328
    Top = 8
  end
  object ADOQuery2: TADOQuery
    Connection = DMHrdsys.conn
    CommandTimeout = 0
    Parameters = <>
    SQL.Strings = (
      '')
    Left = 296
    Top = 8
  end
  object ADOQuery3: TADOQuery
    Connection = DMHrdsys.conn
    CommandTimeout = 0
    Parameters = <>
    Left = 376
    Top = 8
  end
  object ADOQuery4: TADOQuery
    Connection = DMHrdsys.conn
    CommandTimeout = 0
    Parameters = <>
    Left = 448
    Top = 8
  end
  object ADOQueryInq: TADOQuery
    Connection = DMHrdsys.conn
    OnCalcFields = ADOQueryInqCalcFields
    CommandTimeout = 0
    Parameters = <>
    SQL.Strings = (
      'select top 1 * from hrd_dut_day A')
    Left = 144
    Top = 8
    object ADOQueryInqupd_user: TStringField
      FieldName = 'upd_user'
      Size = 16
    end
    object ADOQueryInqupd_date: TDateTimeField
      FieldName = 'upd_date'
    end
    object ADOQueryInqwrk_date: TDateTimeField
      FieldName = 'wrk_date'
    end
    object ADOQueryInqemp_id: TStringField
      FieldName = 'emp_id'
      FixedChar = True
      Size = 6
    end
    object ADOQueryInqseg_code: TStringField
      FieldName = 'seg_code'
      FixedChar = True
      Size = 2
    end
    object ADOQueryInqclas_code: TStringField
      FieldName = 'clas_code'
      FixedChar = True
      Size = 2
    end
    object ADOQueryInqs_start: TStringField
      FieldName = 's_start'
      FixedChar = True
      Size = 4
    end
    object ADOQueryInqs_end: TStringField
      FieldName = 's_end'
      FixedChar = True
      Size = 4
    end
    object ADOQueryInqreal_start: TStringField
      FieldName = 'real_start'
      FixedChar = True
      Size = 4
    end
    object ADOQueryInqreal_end: TStringField
      FieldName = 'real_end'
      FixedChar = True
      Size = 4
    end
    object ADOQueryInqwrk_sts: TStringField
      FieldName = 'wrk_sts'
      FixedChar = True
      Size = 2
    end
    object ADOQueryInqlate: TStringField
      FieldName = 'late'
      FixedChar = True
      Size = 2
    end
    object ADOQueryInqearly: TStringField
      FieldName = 'early'
      FixedChar = True
      Size = 2
    end
    object ADOQueryInqover_flag: TStringField
      FieldName = 'over_flag'
      FixedChar = True
      Size = 1
    end
    object ADOQueryInqnotify: TStringField
      FieldName = 'notify'
      FixedChar = True
      Size = 1
    end
    object ADOQueryInqclas_name: TStringField
      FieldKind = fkCalculated
      FieldName = 'clas_name'
      Calculated = True
    end
    object ADOQueryInqlocal_sts: TStringField
      FieldName = 'local_sts'
      Size = 2
    end
  end
end
