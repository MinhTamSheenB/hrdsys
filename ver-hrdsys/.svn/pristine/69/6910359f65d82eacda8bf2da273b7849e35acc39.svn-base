object FormHrd12: TFormHrd12
  Left = 213
  Top = 149
  Width = 795
  Height = 541
  Caption = 'FormHrd12'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnCreate = FormCreate
  DesignSize = (
    787
    507)
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 787
    Height = 507
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 0
    object PanelTitle: TTntPanel
      Left = 0
      Top = 0
      Width = 787
      Height = 36
      Align = alTop
      BevelOuter = bvNone
      Caption = 'hrd12_titl'
      Color = clCream
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object PanelMain: TPanel
      Left = 0
      Top = 36
      Width = 787
      Height = 471
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 1
      object PanelQuery: TPanel
        Left = 0
        Top = 0
        Width = 787
        Height = 65
        Align = alTop
        TabOrder = 0
        object TntGroupBox1: TTntGroupBox
          Left = 1
          Top = 1
          Width = 785
          Height = 63
          Align = alClient
          Caption = 'query_condition'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          object TntLabel6: TTntLabel
            Left = 5
            Top = 17
            Width = 80
            Height = 13
            Alignment = taRightJustify
            AutoSize = False
            Caption = 'fld_emp_id'
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlue
            Font.Height = -13
            Font.Name = 'MS PGothic'
            Font.Style = []
            ParentFont = False
          end
          object TntLabel10: TTntLabel
            Left = 220
            Top = 14
            Width = 79
            Height = 13
            Alignment = taRightJustify
            AutoSize = False
            Caption = 'fld_department'
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'Palatino Linotype'
            Font.Style = []
            ParentFont = False
          end
          object TntLabel14: TTntLabel
            Left = 5
            Top = 39
            Width = 80
            Height = 13
            Alignment = taRightJustify
            AutoSize = False
            Caption = 'fld_cont_year'
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlue
            Font.Height = -13
            Font.Name = 'MS PGothic'
            Font.Style = []
            ParentFont = False
          end
          object TntLabel50: TTntLabel
            Left = 220
            Top = 38
            Width = 80
            Height = 13
            Alignment = taRightJustify
            AutoSize = False
            Caption = 'fld_info_code'
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlue
            Font.Height = -13
            Font.Name = 'MS PGothic'
            Font.Style = []
            ParentFont = False
            Visible = False
          end
          object edit_begin: TTntEdit
            Left = 89
            Top = 13
            Width = 60
            Height = 21
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
            MaxLength = 6
            ParentFont = False
            TabOrder = 0
          end
          object edit_end: TTntEdit
            Left = 154
            Top = 13
            Width = 60
            Height = 21
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
            MaxLength = 6
            ParentFont = False
            TabOrder = 1
          end
          object Btn_Search: TTntBitBtn
            Left = 544
            Top = 14
            Width = 81
            Height = 25
            Caption = '(&q)uery'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 2
            OnClick = Btn_SearchClick
            Glyph.Data = {
              
              5990555555555555577755555555555555555555555555555555}
            NumGlyphs = 2
          end
          object CobDeptBegin: TTntComboBox
            Left = 302
            Top = 13
            Width = 120
            Height = 21
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            ItemHeight = 13
            ParentFont = False
            TabOrder = 3
          end
          object CobDeptEnd: TTntComboBox
            Left = 424
            Top = 14
            Width = 120
            Height = 21
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
            ItemHeight = 13
            ParentFont = False
            TabOrder = 4
          end
          object BtnPreview: TTntButton
            Left = 627
            Top = 14
            Width = 75
            Height = 25
            Caption = 'preview'
            Enabled = False
            TabOrder = 5
            OnClick = BtnPreviewClick
          end
          object BtnPrint: TTntBitBtn
            Left = 702
            Top = 14
            Width = 75
            Height = 25
            Caption = 'print'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 6
            OnClick = BtnPreviewClick
          end
          object edit_cont_year: TTntEdit
            Left = 89
            Top = 35
            Width = 60
            Height = 21
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
            MaxLength = 6
            ParentFont = False
            TabOrder = 7
          end
          object CobInfo: TTntComboBox
            Left = 302
            Top = 35
            Width = 100
            Height = 21
            Style = csDropDownList
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
            ItemHeight = 13
            ParentFont = False
            TabOrder = 8
            Visible = False
          end
          object CobPst: TTntComboBox
            Left = 424
            Top = 38
            Width = 89
            Height = 21
            Style = csDropDownList
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
            ItemHeight = 13
            ParentFont = False
            TabOrder = 9
          end
        end
      end
      object PanelGrid: TPanel
        Left = 0
        Top = 65
        Width = 787
        Height = 356
        Align = alClient
        TabOrder = 1
        object DBGrid1: TTntDBGrid
          Left = 1
          Top = 1
          Width = 785
          Height = 354
          Align = alClient
          DataSource = DataSource1
          FixedColor = clSkyBlue
          ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'cont_year'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'emp_id'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'name_vim'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'department'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'epindt'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'cont_date1'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'cont_pay1'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'cont_date2'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'cont_pay2'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'cont_date3'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'cont_pay3'
              Visible = True
            end>
        end
      end
      object PanelAct: TTntPanel
        Left = 0
        Top = 421
        Width = 787
        Height = 50
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 2
        object sb_Add: TTntSpeedButton
          Left = 230
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&a)dd'
          Enabled = False
          Flat = True
          Glyph.Data = {
            
            88808888888888888880}
          OnClick = sb_AddClick
        end
        object sb_Recover: TTntSpeedButton
          Left = 470
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&u)ndo'
          Enabled = False
          Flat = True
          Glyph.Data = {
            
            3333333777333777333333333999993333333333377777333333}
          NumGlyphs = 2
          OnClick = sb_RecoverClick
        end
        object sb_Save: TTntSpeedButton
          Left = 550
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&s)ave'
          Enabled = False
          Flat = True
          Glyph.Data = {
            
            0003737FFFFFFFFF7F7330099999999900333777777777777733}
          NumGlyphs = 2
          OnClick = sb_SaveClick
        end
        object sb_edit: TTntSpeedButton
          Left = 390
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&m)odify'
          Enabled = False
          Flat = True
          Glyph.Data = {
            
            00333377737FFFFF773333303300000003333337337777777333}
          NumGlyphs = 2
          OnClick = sb_editClick
        end
        object sb_Del: TTntSpeedButton
          Left = 310
          Top = 12
          Width = 80
          Height = 25
          Caption = '(&d)elete'
          Enabled = False
          Flat = True
          Glyph.Data = {
            
            F000}
          OnClick = sb_DelClick
        end
        object lab_total: TTntLabel
          Left = 5
          Top = 21
          Width = 80
          Height = 13
          Alignment = taRightJustify
          AutoSize = False
          Caption = 'Total:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlue
          Font.Height = -13
          Font.Name = 'MS PGothic'
          Font.Style = []
          ParentFont = False
        end
        object TntBitBtn1: TTntBitBtn
          Left = 702
          Top = 8
          Width = 75
          Height = 25
          Caption = 'print'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          OnClick = TntBitBtn1Click
        end
      end
    end
  end
  object PanelFlow: TTntPanel
    Left = 272
    Top = 64
    Width = 345
    Height = 393
    Anchors = [akLeft, akTop, akRight, akBottom]
    BevelInner = bvRaised
    BevelOuter = bvNone
    BevelWidth = 2
    BorderWidth = 1
    BorderStyle = bsSingle
    TabOrder = 1
    Visible = False
    object TntLabel3: TTntLabel
      Left = 120
      Top = 12
      Width = 98
      Height = 13
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object PanelFlowTitle: TTntPanel
      Left = 3
      Top = 3
      Width = 335
      Height = 24
      Align = alTop
      Alignment = taLeftJustify
      BevelInner = bvLowered
      BevelOuter = bvNone
      Caption = 'edit_area'
      Color = clGradientActiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindow
      Font.Height = -11
      Font.Name = 'Palatino Linotype'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      object ImageClose: TImage
        Left = 313
        Top = 1
        Width = 21
        Height = 22
        Align = alRight
        Picture.Data = {
          
          00007F111508713B5A3B1F76045A5A3B560E1F1E04117F000000}
        OnClick = ImageCloseClick
      end
    end
    object TntGroupBox5: TTntGroupBox
      Left = 13
      Top = 37
      Width = 315
      Height = 309
      Align = alClient
      TabOrder = 0
      object Label_emp_id: TTntLabel
        Left = 5
        Top = 18
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_emp_id'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label_s_ask_h: TTntLabel
        Left = 5
        Top = 119
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_cont_date1'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label_e_ask_h: TTntLabel
        Left = 5
        Top = 140
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_cont_pay1'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel2: TTntLabel
        Left = 5
        Top = 46
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_name_vim'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel5: TTntLabel
        Left = 5
        Top = 73
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_department'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel9: TTntLabel
        Left = 5
        Top = 166
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_cont_date2'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel1: TTntLabel
        Left = 5
        Top = 97
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_epindt'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel4: TTntLabel
        Left = 5
        Top = 190
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_cont_pay2'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel7: TTntLabel
        Left = 5
        Top = 214
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_cont_date3'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel11: TTntLabel
        Left = 5
        Top = 238
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_cont_pay3'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel12: TTntLabel
        Left = 124
        Top = 19
        Width = 100
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'fld_cont_year'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel13: TTntLabel
        Left = 298
        Top = 21
        Width = 15
        Height = 16
        AutoSize = False
        Caption = '*'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object TntLabel8: TTntLabel
        Left = 179
        Top = 21
        Width = 15
        Height = 16
        AutoSize = False
        Caption = '*'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object EditFlow0: TTntDBEdit
        Left = 110
        Top = 16
        Width = 67
        Height = 21
        DataField = 'emp_id'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        TabOrder = 0
        OnKeyDown = EditFlow0KeyDown
      end
      object TntDBEdit2: TTntDBEdit
        Left = 110
        Top = 41
        Width = 147
        Height = 21
        Color = clSkyBlue
        DataField = 'name_vim'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        ReadOnly = True
        TabOrder = 1
      end
      object TntDBEdit4: TTntDBEdit
        Left = 110
        Top = 67
        Width = 145
        Height = 21
        Color = clSkyBlue
        DataField = 'department'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        ReadOnly = True
        TabOrder = 2
      end
      object TntDBEdit3: TTntDBEdit
        Left = 110
        Top = 91
        Width = 83
        Height = 21
        Color = clSkyBlue
        DataField = 'epindt'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        ReadOnly = True
        TabOrder = 3
      end
      object TntDBEdit1: TTntDBEdit
        Left = 110
        Top = 115
        Width = 83
        Height = 21
        Color = clWhite
        DataField = 'cont_date1'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        TabOrder = 4
      end
      object TntDBEdit6: TTntDBEdit
        Left = 110
        Top = 139
        Width = 67
        Height = 21
        Color = clWhite
        DataField = 'cont_pay1'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        TabOrder = 5
      end
      object TntDBEdit5: TTntDBEdit
        Left = 110
        Top = 163
        Width = 83
        Height = 21
        Color = clWhite
        DataField = 'cont_date2'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        TabOrder = 6
      end
      object TntDBEdit7: TTntDBEdit
        Left = 110
        Top = 187
        Width = 67
        Height = 21
        Color = clWhite
        DataField = 'cont_pay2'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        TabOrder = 7
      end
      object TntDBEdit8: TTntDBEdit
        Left = 110
        Top = 211
        Width = 83
        Height = 21
        Color = clWhite
        DataField = 'cont_date3'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        TabOrder = 8
      end
      object TntDBEdit9: TTntDBEdit
        Left = 110
        Top = 235
        Width = 67
        Height = 21
        Color = clWhite
        DataField = 'cont_pay3'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        TabOrder = 9
      end
      object Button1: TButton
        Left = 192
        Top = 114
        Width = 24
        Height = 25
        Caption = '...'
        TabOrder = 10
        OnClick = Button1Click
      end
      object Button2: TButton
        Left = 192
        Top = 162
        Width = 24
        Height = 25
        Caption = '...'
        TabOrder = 11
        OnClick = Button2Click
      end
      object Button3: TButton
        Left = 192
        Top = 210
        Width = 24
        Height = 25
        Caption = '...'
        TabOrder = 12
        OnClick = Button3Click
      end
      object MonthCalendar1: TMonthCalendar
        Left = 0
        Top = 144
        Width = 267
        Height = 154
        Date = 40792.341869571760000000
        ImeName = #20013#25991' (?'#20307') - '#35895#27468#25340#38899'?'#20837#27861
        TabOrder = 13
        Visible = False
        OnDblClick = MonthCalendar1DblClick
      end
      object EditFlow1: TTntDBEdit
        Left = 227
        Top = 16
        Width = 67
        Height = 21
        DataField = 'cont_year'
        DataSource = DataSource1
        ImeName = #19975#33021#20116#31558'EXE'#22806#25346#29256
        TabOrder = 14
      end
    end
    object Panel2: TPanel
      Left = 3
      Top = 346
      Width = 335
      Height = 40
      Align = alBottom
      BevelOuter = bvNone
      TabOrder = 2
      object DBNavigator1: TDBNavigator
        Left = 31
        Top = 8
        Width = 112
        Height = 25
        DataSource = DataSource1
        VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
        Flat = True
        TabOrder = 0
        Visible = False
      end
      object bbtn_add: TTntBitBtn
        Left = 80
        Top = 8
        Width = 65
        Height = 25
        Caption = 'add'
        TabOrder = 1
        OnClick = bbtn_addClick
        Glyph.Data = {
          
          88808888888888888880}
      end
      object bbtn_ok: TTntBitBtn
        Left = 154
        Top = 8
        Width = 65
        Height = 25
        Caption = 'finish'
        TabOrder = 2
        OnClick = bbtn_okClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
    end
    object Panel3: TPanel
      Left = 3
      Top = 27
      Width = 335
      Height = 10
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 3
    end
    object Panel4: TPanel
      Left = 3
      Top = 37
      Width = 10
      Height = 309
      Align = alLeft
      BevelOuter = bvNone
      TabOrder = 4
    end
    object Panel5: TPanel
      Left = 328
      Top = 37
      Width = 10
      Height = 309
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 5
    end
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    Left = 232
  end
  object ADOQuery1: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    AfterOpen = ADOQuery1AfterOpen
    AfterClose = ADOQuery1AfterClose
    BeforePost = ADOQuery1BeforePost
    OnCalcFields = ADOQuery1CalcFields
    OnWillChangeRecord = ADOQuery1WillChangeRecord
    CommandTimeout = 0
    Parameters = <>
    SQL.Strings = (
      'select top 1 * from hrd_lanu A')
    Left = 200
    Top = 2
    object ADOQuery1upd_user: TStringField
      FieldName = 'upd_user'
      Size = 16
    end
    object ADOQuery1upd_date: TDateTimeField
      FieldName = 'upd_date'
    end
    object ADOQuery1emp_id: TStringField
      FieldName = 'emp_id'
      OnChange = ADOQuery1emp_idChange
      Size = 6
    end
    object ADOQuery1name_vim: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'name_vim'
      Size = 25
      Calculated = True
    end
    object ADOQuery1department: TWideStringField
      FieldKind = fkCalculated
      FieldName = 'department'
      Calculated = True
    end
    object ADOQuery1epbirth: TDateTimeField
      FieldKind = fkCalculated
      FieldName = 'epbirth'
      Calculated = True
    end
    object ADOQuery1epindt: TDateTimeField
      FieldKind = fkCalculated
      FieldName = 'epindt'
      Calculated = True
    end
    object ADOQuery1birth_addr: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'birth_addr'
      Calculated = True
    end
    object ADOQuery1pst_name: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'pst_name'
      Calculated = True
    end
    object ADOQuery1tpaddrvim: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'tpaddrvim'
      Size = 100
      Calculated = True
    end
    object ADOQuery1epidno: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'epidno'
      Calculated = True
    end
    object ADOQuery1epid_vim: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'epid_vim'
      Size = 30
      Calculated = True
    end
    object ADOQuery1epid_date: TDateTimeField
      FieldKind = fkCalculated
      FieldName = 'epid_date'
      Calculated = True
    end
    object ADOQuery1cont_date1: TDateTimeField
      FieldName = 'cont_date1'
    end
    object ADOQuery1cont_pay1: TFloatField
      FieldName = 'cont_pay1'
    end
    object ADOQuery1cont_date2: TDateTimeField
      FieldName = 'cont_date2'
    end
    object ADOQuery1cont_pay2: TFloatField
      FieldName = 'cont_pay2'
    end
    object ADOQuery1cont_date3: TDateTimeField
      FieldName = 'cont_date3'
    end
    object ADOQuery1cont_pay3: TFloatField
      FieldName = 'cont_pay3'
    end
    object ADOQuery1book_id: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'book_id'
      Calculated = True
    end
    object ADOQuery1book_area: TTntWideStringField
      DisplayWidth = 30
      FieldKind = fkCalculated
      FieldName = 'book_area'
      Size = 30
      Calculated = True
    end
    object ADOQuery1cont_num: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'cont_num'
      Calculated = True
    end
    object ADOQuery1cont_date: TDateTimeField
      FieldKind = fkCalculated
      FieldName = 'cont_date'
      Calculated = True
    end
    object ADOQuery1cont_id: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'cont_id'
      Calculated = True
    end
    object ADOQuery1country: TTntWideStringField
      FieldKind = fkCalculated
      FieldName = 'country'
      Calculated = True
    end
    object ADOQuery1cont_year: TStringField
      FieldName = 'cont_year'
      Size = 4
    end
    object ADOQuery1book_date: TStringField
      FieldKind = fkCalculated
      FieldName = 'book_date'
      Size = 10
      Calculated = True
    end
  end
  object PrintDialog1: TPrintDialog
    Left = 640
    Top = 4
  end
end
