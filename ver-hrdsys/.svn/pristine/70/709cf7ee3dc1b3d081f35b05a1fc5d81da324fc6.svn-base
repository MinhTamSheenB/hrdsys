...
type
  ...
  private
    ...
    //2015.12.15 得到保險類別
    function GetInsurClass(AClassName: string): string;
    //標準工號 2015.12.23
    function GetStdEmpID(AEmpID: string): string;
...
function TFormHrd72.GetInsurClass(AClassName: string): string;
var
  sSQL: string;
  //
  sRate : string;
begin
  ...
  sRate := AClassName;
  //
  sRate := StringReplace(sRate, '0.', '', [rfReplaceAll]);
  //
  sRate := StringReplace(sRate, '%', '', [rfReplaceAll]);
  //
  if sRate= '1' then
    sRate := '100';
  sSQL := 'select clas_code from HRD_EMP_INSURANCE_CLASS where clas_rate= '+QuotedStr(sRate);
  with DMHrdsys.SQLQuery4 do
  begin
    Close;
    SQL.Clear;
    ...
    Open;
    if not IsEmpty then
      Result := FieldByName('clas_code').AsString;
    Close;
  end;
end;

procedure TFormHrd72.ImportDataFromExcel;
var
  sFileName: string;
  sMonth: string;
  //
  ...
  iRows, I: Integer;
begin
  sMonth := InputBox('Month', 'Month', FormatDateTime('YYYYMM', Date));
  //
  if Trim(sMonth) = '' then Exit;
  //
  if not OpenExcel.Execute then Exit;
  //
  sFileName := OpenExcel.FileName;
  //
  ...
  try
    ...
    begin
      //
      if Trim(MyExcel.Cells[I, 2]) = '' then Break;
      ADOQuery1.Append;
      //保險月份
      ADOQuery1.FieldByName('insur_month').AsString := sMonth;
      //工號
      ADOQuery1.FieldByName('emp_id').AsString := GetStdEmpID(MyExcel.Cells[I, 2]);
      //日期起
      ADOQuery1.FieldByName('date_b').AsString := MyExcel.Cells[I, 4];
      //日期止
      ADOQuery1.FieldByName('date_e').AsString := MyExcel.Cells[I, 5];
      //參保天數
      ADOQuery1.FieldByName('insur_days').AsString := MyExcel.Cells[I, 6];
      //金額
      ADOQuery1.FieldByName('insur_money').AsString := MyExcel.Cells[I, 7];
      //備註
      ADOQuery1.FieldByName('remark').AsString := MyExcel.Cells[I, 8];
      //申請日期
      ADOQuery1.FieldByName('apply_date').AsString := MyExcel.Cells[I, 9];
    end;
  finally
    MyExcel.Quit;
    MyExcel := Unassigned;
...
procedure TFormHrd72.sb_ImportClick(Sender: TObject);
begin
  with adoquery1 do
  begin
    if not active then
...
//標準工號 2015.12.23
function TFormHrd72.GetStdEmpID(AEmpID: string): string;
var
  sEmpID: string;
  //
  I, iLen: Integer;
begin
  ...
  I := 6 - iLen;
  //
  Result := StringOfChar('0', I)+sEmpID;
end;
