...
implementation
uses
  UnitHrdUtils,unitDMHrdsys, UnitPrtDut12 ;
var
      ...
      begin
         if CLB_DTL.State[i]=cbChecked then
            begin
              ...
              if RBOver.Checked then
              begin
                {sql := 'SELECT  (select name_vim_en from hrd_emp where emp_id=b.emp_id) as name_vim_en,'+
                        ' SUM(CONVERT(float, LEFT(c.s_end, 2)) - CONVERT(float,'+
                        '  LEFT(c.s_start, 2)) + (CONVERT(float, RIGHT(c.s_end, 2)) - CONVERT(float,'+
                        '  RIGHT(c.s_start, 2))) / 60) AS ot_year '+
                     'FROM hrd_dut_ovrmst a INNER JOIN '+
                          'hrd_dut_ovrdtl b ON a.batch_no = b.batch_no INNER JOIN '+
                          'hrd_dut_seg c ON a.seg_code = c.seg_code '+
                     'WHERE  (a.over_date >= ''2011/01/01'') AND (a.over_date <= ''2011/12/31'') AND '+
                          '(b.emp_id = '''+emp+''')'+
                     ' GROUP BY  b.emp_id';
                     }  //2013-09-04 hyt upd
      ...
      with ADOQuery1 do
        begin
          ...
          if not EOF then
            //顯示部門
         //   with DMHrdsys.SQLQuery1 do
              FormPrtDut12.QRLabelDept.Caption:=DMHrdsys.SQLQuery1.fieldByName('dept_code').Value+' '+DMHrdsys.SQLQuery1.fieldByName('abbr_titl').Value;
            //按部門打印/預覽
