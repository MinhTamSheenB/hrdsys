...
type
  TFormDut39 = class(TForm)
    ...
    procedure edit_endChange(Sender: TObject);
...
procedure TFormDut39.edit_endChange(Sender: TObject);
begin
  if not Chk_Emp.Checked then
    Chk_Emp.Checked := True;
end;

procedure TFormDut39.edit_beginChange(Sender: TObject);
begin
  if not Chk_Emp.Checked then
    Chk_Emp.Checked := True;
  //  
