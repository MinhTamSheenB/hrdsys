object FormDut18: TFormDut18
  Left = 316
  Top = 150
  Width = 800
  Height = 570
  Caption = 'FormDut18'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Scaled = False
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 792
    Height = 536
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 0
    object PanelTitle: TTntPanel
      Left = 0
      Top = 0
      Width = 792
      Height = 49
      Align = alTop
      BevelOuter = bvNone
      Caption = 'dut18_titl'
      Color = clCream
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -16
      Font.Name = 'Palatino Linotype'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object PanelMain: TPanel
      Left = 0
      Top = 49
      Width = 792
      Height = 487
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 1
      object TntGroupBox1: TTntGroupBox
        Left = 0
        Top = 0
        Width = 792
        Height = 73
        Align = alTop
        Caption = 'search_condition'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object TntLabel3: TTntLabel
          Left = 380
          Top = 22
          Width = 86
          Height = 16
          Caption = 'dbl_clk_slct_emp'
          Color = clYellow
          ParentColor = False
        end
        object TntLabel1: TTntLabel
          Left = 8
          Top = 33
          Width = 69
          Height = 16
          Alignment = taRightJustify
          AutoSize = False
          Caption = 'fld_year'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
        end
        object CobDeptBegin: TTntComboBox
          Left = 237
          Top = 45
          Width = 181
          Height = 24
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          ItemHeight = 16
          ParentFont = False
          TabOrder = 0
          OnChange = CobDeptBeginChange
        end
        object CobDeptEnd: TTntComboBox
          Left = 420
          Top = 45
          Width = 180
          Height = 24
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          ItemHeight = 16
          ParentFont = False
          TabOrder = 1
        end
        object bb_reset: TTntBitBtn
          Left = 696
          Top = 13
          Width = 83
          Height = 25
          BiDiMode = bdRightToLeftNoAlign
          Caption = 'reset'
          Default = True
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ModalResult = 4
          ParentBiDiMode = False
          ParentFont = False
          TabOrder = 2
          OnClick = bb_resetClick
          Glyph.Data = {
            
            0000}
          NumGlyphs = 2
        end
        object bb_ok: TTntBitBtn
          Left = 608
          Top = 14
          Width = 83
          Height = 25
          Caption = 'ok'
          Default = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ModalResult = 1
          ParentFont = False
          TabOrder = 3
          OnClick = bb_okClick
          Glyph.Data = {
            
            0000}
          NumGlyphs = 2
        end
        object Btn_Print: TTntBitBtn
          Left = 609
          Top = 42
          Width = 83
          Height = 25
          Caption = '(&p)rint'
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
          TabOrder = 4
          OnClick = Btn_PrintClick
          Glyph.Data = {
            
            33333337FFFF7733333333300000033333333337777773333333}
          NumGlyphs = 2
        end
        object EditEmpBegin: TTntEdit
          Left = 238
          Top = 16
          Width = 65
          Height = 24
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          MaxLength = 6
          ParentFont = False
          TabOrder = 5
          OnChange = EditEmpBeginChange
          OnDblClick = EditEmpBeginDblClick
        end
        object EditEmpEnd: TTntEdit
          Left = 310
          Top = 16
          Width = 65
          Height = 24
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          MaxLength = 6
          ParentFont = False
          TabOrder = 6
          OnDblClick = EditEmpBeginDblClick
        end
        object TntCheckBox1: TTntCheckBox
          Left = 136
          Top = 22
          Width = 97
          Height = 17
          Caption = 'fld_emp_id'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
          TabOrder = 7
          OnClick = TntCheckBox1Click
        end
        object TntCheckBox2: TTntCheckBox
          Left = 136
          Top = 48
          Width = 97
          Height = 17
          Caption = 'dept_range'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ParentFont = False
          TabOrder = 8
          OnClick = TntCheckBox1Click
        end
        object me_year: TMaskEdit
          Left = 81
          Top = 31
          Width = 32
          Height = 24
          EditMask = '0000;1; '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Palatino Linotype'
          Font.Style = []
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          MaxLength = 4
          ParentFont = False
          TabOrder = 9
          Text = '    '
          OnClick = me_yearClick
        end
      end
      object TntDBGrid1: TTntDBGrid
        Left = 0
        Top = 73
        Width = 792
        Height = 370
        Align = alClient
        DataSource = DataSource1
        FixedColor = clSkyBlue
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Palatino Linotype'
        Font.Style = []
        ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
        ParentFont = False
        ReadOnly = True
        TabOrder = 1
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'emp_id'
            Width = 44
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'emp_chs'
            Width = 48
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'dept_code'
            Width = 56
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'abbr_titl'
            Width = 123
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'epindt'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CanRestYearHoliday'
            Width = 66
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SpringFestivalHoliday'
            Width = 79
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RestYearHoliday_17'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RestYearHoliday'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'remaindays'
            Width = 59
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SpringFestTime'
            Visible = True
          end>
      end
      object TntGroupBox2: TTntGroupBox
        Left = 0
        Top = 443
        Width = 792
        Height = 44
        Align = alBottom
        Caption = 'total'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        object TntLabel2: TTntLabel
          Left = 12
          Top = 18
          Width = 89
          Height = 13
          Alignment = taRightJustify
          AutoSize = False
          Caption = 'peoplesum'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object TntLabel4: TTntLabel
          Left = 188
          Top = 18
          Width = 89
          Height = 13
          Alignment = taRightJustify
          AutoSize = False
          Caption = 'peoplecan'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object TntLabel5: TTntLabel
          Left = 360
          Top = 18
          Width = 89
          Height = 13
          Alignment = taRightJustify
          AutoSize = False
          Caption = 'RestYearHoliday'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object TntLabel6: TTntLabel
          Left = 538
          Top = 19
          Width = 89
          Height = 13
          Alignment = taRightJustify
          AutoSize = False
          Caption = 'remaindays'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object TntEdit1: TTntEdit
          Left = 107
          Top = 12
          Width = 65
          Height = 21
          Enabled = False
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          TabOrder = 0
        end
        object TntEdit2: TTntEdit
          Left = 283
          Top = 12
          Width = 65
          Height = 21
          Enabled = False
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          TabOrder = 1
        end
        object TntEdit3: TTntEdit
          Left = 454
          Top = 12
          Width = 65
          Height = 21
          Enabled = False
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          TabOrder = 2
        end
        object TntEdit4: TTntEdit
          Left = 632
          Top = 12
          Width = 65
          Height = 21
          Enabled = False
          ImeName = 'Chinese (Simplified) - Microsoft Pinyin IME 3.0'
          TabOrder = 3
        end
      end
    end
  end
  object DataSource1: TDataSource
    DataSet = ClientDataSet1
    Left = 288
  end
  object ADOQuery1: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <>
    Prepared = True
    SQL.Strings = (
      ''
      
        'select a.emp_id,a.emp_chs,a.dept_code,b.abbr_titl,a.epindt from ' +
        'hrd_emp a,hrd_dept b where a.dept_code=b.dept_code')
    Left = 168
    object ADOQuery1emp_id: TStringField
      FieldName = 'emp_id'
      Size = 6
    end
    object ADOQuery1emp_chs: TWideStringField
      FieldName = 'emp_chs'
      Size = 10
    end
    object ADOQuery1dept_code: TStringField
      FieldName = 'dept_code'
      Size = 6
    end
    object ADOQuery1abbr_titl: TWideStringField
      FieldName = 'abbr_titl'
      Size = 14
    end
    object ADOQuery1epindt: TDateTimeField
      FieldName = 'epindt'
    end
  end
  object PrintDialog1: TPrintDialog
    Left = 665
    Top = 9
  end
  object ADOQuery2: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <>
    Prepared = True
    SQL.Strings = (
      'select '
      'a.emp_id,'
      'a.emp_chs,'
      'a.dept_code,'
      'b.abbr_titl,'
      'a.epindt,'
      'a.name_eng as CanRestYearHoliday,'
      'a.name_eng as SpringFestivalHoliday,'
      'a.name_eng as RestYearHoliday,'
      'a.name_eng as remaindays'
      ''
      ' from hrd_emp a,hrd_dept b '
      'where a.dept_code=b.dept_code ')
    Left = 344
    Top = 8
  end
  object ClientDataSet1: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 240
    object ClientDataSet1emp_id: TStringField
      DisplayWidth = 7
      FieldName = 'emp_id'
      Size = 6
    end
    object ClientDataSet1emp_chs: TWideStringField
      DisplayWidth = 9
      FieldName = 'emp_chs'
      Size = 10
    end
    object ClientDataSet1dept_code: TStringField
      DisplayWidth = 10
      FieldName = 'dept_code'
      Size = 6
    end
    object ClientDataSet1abbr_titl: TWideStringField
      DisplayWidth = 17
      FieldName = 'abbr_titl'
      Size = 14
    end
    object ClientDataSet1epindt: TDateTimeField
      DisplayWidth = 15
      FieldName = 'epindt'
    end
    object ClientDataSet1CanRestYearHoliday: TWideStringField
      DisplayWidth = 24
      FieldName = 'CanRestYearHoliday'
    end
    object ClientDataSet1SpringFestivalHoliday: TWideStringField
      DisplayWidth = 24
      FieldName = 'SpringFestivalHoliday'
    end
    object ClientDataSet1RestYearHoliday_17: TWideStringField
      FieldName = 'RestYearHoliday_17'
    end
    object ClientDataSet1RestYearHoliday: TWideStringField
      DisplayWidth = 24
      FieldName = 'RestYearHoliday'
    end
    object ClientDataSet1remaindays: TWideStringField
      DisplayWidth = 13
      FieldName = 'remaindays'
    end
    object ClientDataSet1SpringFestTime: TFloatField
      FieldName = 'SpringFestTime'
    end
  end
  object qry_pub: TADOQuery
    Connection = DMHrdsys.conn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <>
    Prepared = True
    Left = 440
    Top = 8
  end
end
