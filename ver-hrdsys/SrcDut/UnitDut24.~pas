...
begin
  ...
  begin
    ...
    sql.Add('select * from hrd_emp where ((epledt is null)or(epledt>'''+
          ...
          ''')) and dept_code='''+leftstr(cb_dept.Text,6)+''' order by emp_id');
    open;
    clb_emp.Items.Clear;
    i:=0;
    while not eof do
...
begin
  if not checkinput then
    exit;
  //
  sDutClass := Copy(cbbClass.Text, 1, 2);  
  try
    ...
    begin
      ...
      begin
        ...
        SqlStr:=SqlStr+'insert into hrd_dut_ask'+
             ...
             ' values('''+UserName+''','''+timestr+''','''+emp_id+''','+QuotedStr(sDutClass)+','''+
...
begin
  ...
  begin
    ...
    sql.Add('select * from hrd_dut_ask where emp_id='''+temp_id+''''
      +' and (clas_code=''16'' or clas_code=''17'') and YEAR(s_ask_d)='+IntToStr(YearOf(ask_d)));
    open;
    HasDays:=0;//計算已請年假天數
    while not Eof do
    begin
