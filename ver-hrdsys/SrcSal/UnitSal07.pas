...
begin
  ...
  begin
    ...
    formsal07.re_Message.Lines.Add('ÂàÀÉ§¹²¦¡I');
    formsal07.SetEnableControl(1,true);
  end;
end;
{===============================================================================
  ...
  begin
    ...
    //  F_EnvrPay:=ASalGrads[4];                 //5,Àô¹Ò¬z¶K(§R)
      ...
      begin
        ...
        //³B²z·s¶i§@·~­û¡]Àô¹Ò¬z¶K¡^²Î¤@«ö40000¶V¹ô®Ö¸ê¨Æ©y
        //if copy( deptcode, 1, 1) = 'N' then
        //begin   //¥u¹ï©³¼t
        //  if (PayMon>='201104') and (formatDatetime('yyyymm',EmpInDate)=PayMon) then //¥u¹ï¥»¤ë·s¶i¦³®Ä¡A¦¸¤ë¤£¤ÀªR
        //  begin
...
//      F_DutyPay:=0;
 ...
 // 2010/07/12  >>
  ...
  if IsNewEmp(EmpId,PayMon,PstCode,EmpInDate,EmpLvDate) then      //§PÂ_¸Ó­û¤u¬O§_¬°·s¶i­û¤u
  begin
    if PstCode >= '60' then
  ...
  begin
   ...
   //   begin
    if paymon < '200807' then
      F_GongPay := int((WorkDays/26)*100000 ) //1,¥t¥~¸É¶K
    else
    begin
      if paymon ='200807'  then
        F_GongPay := int((WorkDays/26)*135000)  //1,¥t¥~¸É¶K
      else
      begin
        if paymon <'201103'  then
          F_GongPay := int((WorkDays/26)*200000 ) //1,¥t¥~¸É¶K
        else
          F_GongPay := int((WorkDays/26)*400000 ); //1,¥t¥~¸É¶K   2011¦~3¤ë
      end;
    end;
    //  end;
  end;
...
begin
  //PayNormal:=GetNormalBase(PstCode,EduCode,EmpInDate,SalBaseLvl);//¥¿¦¡°ò¥»Á~¸ê
 // if paymon <'200801'  then
     PayNormal:=GetNormalBase(Empid, paymon, PstCode,EduCode,EmpInDate,SalBaseLvl,false); //¥¿¦¡°ò¥»Á~¸ê
 // else
  //   begin
  //     if( pstcode ='60' )  then
  //        PayNormal:=GetNormalBase(PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 170000 ;   //2008.01 °_Â¾­û§@·~­ûÁ~¸ê¤W¯B190000
  //     if ( pstCode ='50' ) then
  //        PayNormal:=GetNormalBase(PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 190000 ;   //2008.01 °_Â¾­û§@·~­ûÁ~¸ê¤W¯B190000
  //     if ( pstCode ='46' ) or ( pstcode ='41' ) or ( pstcode ='42' )  then
 //         PayNormal:=GetNormalBase(PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 172000 ;  // 2008.01 °_¯Zªø¯ÅÁ~¸ê¤W¯B172000
  //     if ( pstCode ='33' ) or ( pstcode ='37' )  then
  //        PayNormal:=GetNormalBase(PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 242000 ;  // 2008.01 °_²Õªø¯ÅÁ~¸ê¤W¯B242000
  //     if ( pstCode ='22' ) or ( pstcode ='26' ) or ( pstcode ='28' )  then
  //        PayNormal:=GetNormalBase(PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 342000 ;  // 2008.01 °_½Òªø¯ÅÁ~¸ê¤W¯B342000
  //     if ( pstCode ='18' ) or ( pstcode ='19' )   then
  //        PayNormal:=GetNormalBase(PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 442000 ;  // 2008.01 °_¥D¥ô¯ÅÁ~¸ê¤W¯B90000
  //   end;
//  if PstCode>='50' then//¸Õ¥Î´ÁÁ~¸ê
//     begin
       if paymon >='200801'  then
 ...
 //   end
 //   end
//  else//Â¾­û¥H¤WµL¸Õ¥Î´Á
  //  begin
//     if paymon >='200801'  then
  //        PayTest:=PayNormal + 90000    // 2008.01°_¸Õ¥Î´ÁÁ~¸ê¤W¯B90000
   //    else
  //       PayTest:=PayNormal ;
 //   end;
   // PayTest:=PayNormal;
  ...
  if EmpInDate<EncodeDate(2004,9,1) then//2004/9/1«e¶i¼t¦Ò¼{¾Ç¾úªþ¥[ª÷ÃB
  begin
  ...
  //***<<<2014-01-07 hyt add ¤ÀªR­pºâ¥b­Ó¤ëÁ~¸êªº±¡ªp
  {if PayMon='201401' then
  begin
    if (NormalDays+TestDays)>13 then
    begin         //beizhu
      if TestDays>=13 then //¥ý¦©
      begin
        TestDays:=13;
        NormalDays:=0;
      end
...
//  else
 //    begin
  //     if( pstcode ='60' )  then
 //         PayNormal:=GetNormalBase(paymon,PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 170000 ;   //2008.01 °_Â¾­û§@·~­ûÁ~¸ê¤W¯B190000
 //      if ( pstCode ='50' ) then
  //        PayNormal:=GetNormalBase(paymon,PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 190000 ;   //2008.01 °_Â¾­û§@·~­ûÁ~¸ê¤W¯B190000
  //     if ( pstCode ='46' ) or ( pstcode ='41' ) or ( pstcode ='42' )  then
  //        PayNormal:=GetNormalBase(paymon,PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 172000 ;  // 2008.01 °_¯Zªø¯ÅÁ~¸ê¤W¯B172000
  //     if ( pstCode ='33' ) or ( pstcode ='37' )  then
  //        PayNormal:=GetNormalBase(paymon,PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 242000 ;  // 2008.01 °_²Õªø¯ÅÁ~¸ê¤W¯B242000
  //     if ( pstCode ='22' ) or ( pstcode ='26' ) or ( pstcode ='28' )  then
  //        PayNormal:=GetNormalBase(paymon,PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 342000 ;  // 2008.01 °_½Òªø¯ÅÁ~¸ê¤W¯B342000
   //    if ( pstCode ='18' ) or ( pstcode ='19' )   then
   //       PayNormal:=GetNormalBase(paymon,PstCode,EduCode,EmpInDate,SalBaseLvl,false) + 442000 ;  // 2008.01 °_¥D¥ô¯ÅÁ~¸ê¤W¯B90000
 //    end;
 // if PstCode>='50' then//¸Õ¥Î´ÁÁ~¸ê
  //   begin
  //     if paymon >='200801'  then
...
{    with qry_Temp do
    begin
      ...
      begin
        ...
        FirstDay:=ValidDay-GetSunDayCount(1,ValidDay,PayMon);  //¥»¤ë½ÕÁ~«e¤u§@¤Ñ¼Æ
        SecondDay:=WorkDays-FirstDay;                          //¥»¤ë½ÕÁ~¦Z¤u§@¤Ñ¼Æ
...
begin
  ...
  //***<<< 2014-01-07 hyt add ¤ÀªRºâ¥b­Ó¤ëÁ~¸êªº±¡ªp
  {if PayMon='201401' then
  begin    //beizhu
...
var
  NormalTestDays:TNormalTestDays;
  CurServerDateTime:TDateTime;  //ªA°È¾¹¤W·í«e®É¶¡
...
begin
  ...
  begin
    ...
    begin
      ...
      TestDays:=26-(TheInDay-SundayCount)+1; //2010-10-04 hyt add ¥[1¤Ñ,¦]³W©w¤@¶i¼t´N­nºâÁ~¸ê  exp:810740
      NormalDays:=0;
    end
    else begin//·í¤ë§YÂ÷Â¾
      TheLvDay:=DayOf(EmpLvDate)-1;
